...
export function EditListingModal({ isOpen, listing, onClose, onSuccess }: EditListingModalProps) {
    ...
    useEffect(() => {
        if (listing && isOpen) {
            setFormData({
                ...
                maxDistance: listing.maxDistance || 50,
    ...
    if (!isOpen || !formData || !listing) return null;

    const handleNext = () => {
        setError('');
        setIsTransitioning(true);
        setStep(s => Math.min(3, s + 1));
        setTimeout(() => setIsTransitioning(false), 500); // Wait for transition animation
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (isTransitioning) return; // Prevent accidental rapid submits during animation
            ...
            <div className="bg-slate-900 border border-slate-700/80 rounded-[2rem] w-full max-w-3xl shadow-2xl relative animate-slide-up flex flex-col max-h-[90vh]">
                ...
                <div className="shrink-0 bg-slate-900/95 backdrop-blur-xl border-b border-slate-800/80 px-8 py-6 flex flex-col gap-4 rounded-t-[2rem] z-10">
                    ...
                    <div className="flex gap-2">
                        {[1, 2, 3].map((s) => (
                            <div key={s} className={`h-1.5 flex-1 rounded-full transition-colors duration-500 ${s <= step ? 'bg-amber-500 shadow-[0_0_10px_rgba(245,158,11,0.5)]' : 'bg-slate-800'}`} />
                        ))}
                    </div>
                </div>
                        ...
                        <div className={`space-y-6 transition-all duration-500 ${step === 1 ? 'opacity-100 translate-x-0 relative' : 'opacity-0 absolute -translate-x-full pointer-events-none'}`}>
                            <div className="flex justify-between items-end mb-6">
                                <div>
                                    ...
                                    <p className="text-sm text-slate-400">Müşterilerin göreceği ana başlık ve detaylar.</p>
                                </div>
                                
                                {/* Status Toggle */}
                                <div className="flex items-center gap-3 bg-slate-800/50 p-2 rounded-xl border border-slate-700">
                                    <span className="text-sm font-semibold text-slate-300">Durum:</span>
                                    <button
                                        type="button"
                                        onClick={() => setFormData({ ...formData, isActive: !formData.isActive })}
                                        className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2 focus:ring-offset-slate-900 ${formData.isActive ? 'bg-emerald-500' : 'bg-slate-600'}`}
                                    >
                                        <span className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${formData.isActive ? 'translate-x-6' : 'translate-x-1'}`} />
                                    </button>
                                    <span className={`text-xs font-bold ${formData.isActive ? 'text-emerald-400' : 'text-slate-400'}`}>
                            ...
                            <div>
                                ...
                                <div className="relative">
                                    <select className="input-field appearance-none cursor-pointer shadow-inner pr-10" value={formData.category} onChange={e => setFormData({ ...formData, category: Number(e.target.value) as any })}>
                                        ...
                                        <option value={3} className="bg-slate-900">Ticari</option>
                                        <option value={4} className="bg-slate-900">Haritacılık</option>
                                        <option value={5} className="bg-slate-900">Tarım</option>
                                        <option value={6} className="bg-slate-900">İnşaat</option>
                                        <option value={7} className="bg-slate-900">Etkinlik</option>
                                        <option value={8} className="bg-slate-900">Sinematografi</option>
                                    </select>
                ...
                <div className="shrink-0 bg-slate-900 border-t border-slate-800/60 px-8 py-5 rounded-b-[2rem] z-10 flex flex-col gap-4">
                    ...
                    <div className="flex justify-between items-center mt-1">
                        ...
                        <div className="flex gap-3">
                            ...
                            {step < 3 ? (
                                <button type="button" onClick={handleNext} className="py-2.5 px-6 rounded-xl font-semibold bg-amber-600 hover:bg-amber-500 text-white shadow-lg shadow-amber-500/20 active:scale-95 transition-all flex items-center gap-1.5 focus:outline-none focus:ring-2 focus:ring-amber-500/50">
                                    İleri <ChevronRight size={16} />
                                </button>
                            ) : (
                                <button type="submit" form="edit-listing-form" disabled={loading || isTransitioning} className="py-2.5 px-8 rounded-xl font-semibold bg-emerald-500 hover:bg-emerald-400 text-white shadow-lg shadow-emerald-500/20 active:scale-95 transition-all disabled:opacity-50 disabled:active:scale-100 flex items-center gap-2 focus:outline-none focus:ring-2 focus:ring-emerald-500/50">
                                    {loading && <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />}
                                    {loading ? 'Güncelleniyor...' : 'Güncelle Kaydet'}
                                </button>
                            )}
                        </div>
                    </div>
