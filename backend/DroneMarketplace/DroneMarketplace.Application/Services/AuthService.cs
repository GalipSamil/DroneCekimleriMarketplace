...
namespace DroneMarketplace.Application.Services
{
    public class AuthService : IAuthService
    {
        ...
        public async Task ForgotPasswordAsync(string email)
        {
            ...
            var encodedEmail = Uri.EscapeDataString(user.Email ?? email);
            var resetLink = $"{normalizedFrontendBaseUrl}/reset-password?token={encodedToken}&email={encodedEmail}";
            var subject = "Reset your DronePazar password";
            var body =
                "We received a request to reset your DronePazar password.\n\n" +
                $"Reset your password using the link below:\n{resetLink}\n\n" +
                "If you did not request this change, you can ignore this email.";

            await _emailService.SendEmailAsync(user.Email ?? email, subject, body);
        }
